...
! headsym="$(git symbolic-ref -q HEAD)" || [ "$headsym" != "refs/heads/$name" ] || {
	...
	git update-ref -m "tgdelete: detach HEAD to delete $name" --no-deref HEAD "$branchrev" || die "detach failed"
	warn "$(git --no-pager log -n 1 --format=format:'HEAD is now at %h... %s' HEAD)"
}
